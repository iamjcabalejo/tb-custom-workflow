...
2. **Graphify phase budget:** Prefer plan **File changes** (plan-first). Query only for gaps/blast radius (`graphify.mdc` / `graphify-navigation`). Do not load the full `/graphify` skill for routine query. After edits: `graphify update .` in that root when CLI available.
