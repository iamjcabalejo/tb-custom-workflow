...
**Built-in Task subagent types** (Codex `Task` tool or equivalent): `generalPurpose`, `explore`, `shell`, and other `subagent_type` values the runner exposes. These may not have definition files in `.codex/agents/`; use them when the workflow requires general-purpose execution, codebase exploration, or shell commands.
