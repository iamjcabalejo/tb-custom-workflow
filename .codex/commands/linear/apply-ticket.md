...
1. **Before the first MCP call:** discover tool schemas via MCP tools (e.g. GetMcpTools / descriptors)—do not invent tool names.
...
**After fetch:** Refine once (token-policy). Do **not** re-paste full Linear description/comments into later chat turns or subagent prompts—use the refined plan and AC only.
...
**Vague tickets:** Prefer **linear/refine-spec** first (mutates Linear), then re-run apply-ticket—or continue with labeled Assumptions if the user insists.

## Context hygiene (mandatory)

- After writing `docs/plans/<feature-slug>.md`, Code and Review use **plan path + AC + File changes** only.
- For **large** tickets (multi-root, many AC, or Conversation already heavy): after PLAN completes, **stop** and tell the user to run **`/project-manager docs/plans/<feature-slug>.md` in a new chat** for Code → Review. Continue in-thread only for small/medium tickets when context is still light.
- Do not keep stacking MCP payloads, full plan drafts, and tool dumps in the parent chat after the plan file exists.
...
| Phase | Action |
|-------|--------|
| PLAN | Required: `graphify query "…" --budget 1500` (or path/explain) before exploring product repos; write concrete **File changes**. |
| Code | Plan-first; query only for gaps/blast radius. Optional: one orchestrator query per root → short summary in prompts. |
| Review | Diff-first; query only for dependency/blast radius. |
| After edits | `graphify update .` when CLI available. |

Do **not** load the full `/graphify` skill for routine query/path/explain.
...
4. In plan **metadata**, record: `Linear: <KEY>` (e.g. `TB-42`), original title, and link if returned by `get_issue`.
5. **Agent scope (delegate only what applies):**
...
6. **Gate:** Plan is complete when another agent can implement without guessing. If gate fails, stay in PLAN or ASK—do not spawn Code agents.
7. If splitting to a new chat (large ticket): stop here and hand off plan path to `/project-manager`.

## Phase 2: CODE — implement to plan

**Mode:** AGENT for all Code subagents.

Spawn **only** agents needed per the plan. Pass: **plan path** (or relevant sections), acceptance criteria, file changes, API contract, env/deps—**not** the raw Linear body. Each agent follows compounding-dev-cycle **Code** phase.
...
Spawn reviewers **only** for domains that were implemented. Pass plan AC + diff + notes—not Linear dumps.
...
- **feature-plan** / **project-manager** — same cycle when the input is already a plan path, not a ticket id. Prefer **project-manager** in a **new chat** after PLAN for large tickets.
